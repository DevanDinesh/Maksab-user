      ...
      if(_addressList.isNotEmpty)
        {
          ...
          Get.find<AuthController>().authRepo.saveUserId(_addressList[0].userId.toString());
        }
    } else {
  ...
  Future<ResponseModel> addAddress(AddressModel addressModel, bool fromCheckout, int restaurantZoneId) async {
    ...
    if (response.statusCode == 200) {
      if(fromCheckout && !response.body['zone_ids'].contains(restaurantZoneId)) {
        responseModel = ResponseModel(true, 'your_selected_location_is_from_different_zone'.tr);
      }else {
        getAddressList();
