...
class CheckoutScreen extends StatefulWidget {
  ...
  CheckoutScreen({@required this.fromCart, @required this.cartList, @required this.orderDetails});
...
class _CheckoutScreenState extends State<CheckoutScreen> {
                ...
                return GetBuilder<CouponController>(
                  ...
                  return GetBuilder<OrderController>(
                    ...
                    if (restController.restaurant != null &&
                      ...
                      double _minimumCharge = restController
                                  .restaurant.selfDeliverySystem ==
                              1
                      ...
                      print('1');
                      // print(_deliveryCharge.toString()+ "rtrtrtr");
                      ...
                      double _maximumCharge = Get.find<LocationController>()
                          .getUserAddress()
                          .zoneData
                          .firstWhere((data) =>
                              data.id == restController.restaurant.zoneId)
                          .maximumShippingCharge;
                      double minimum_km = Get.find<LocationController>()
                                  .getUserAddress()
                                  .zoneData
                                  .firstWhere((data) =>
                                      data.id ==
                      ...
                      _deliveryCharge = _minimumCharge;
                      _charge = _minimumCharge;
                      if (orderController.distance > minimum_km) {
                        _charge = orderController.distance * _perKmCharge +
                            _minimumCharge;
                      }
                     if (_charge > _maximumCharge) {
                        _charge = _maximumCharge;
                        print(_charge);
                        // print(_charge.toString()+ "rtrtrtr");
                      }
                    }

                    //  double _maximumCharge =
                    //      restController.restaurant.maximumShippingCharge;
                    // else if(_deliveryCharge > _maximumCharge){
                    ...
                    if (restController.restaurant != null &&
                        restController.restaurant.categoryInfo != null &&
                        restController.restaurant.categoryInfo.isNotEmpty &&
                        restController
                                .restaurant.categoryInfo[0].includeDelivery ==
                            1) {
                      _total = _subTotal +
                          _discount -
                          _couponDiscount +
                                                  ...
                                                  child: restController
                                                      ...
                                                      ? Text(
                                                          'delivery_charge'.tr +
                                                              ': ' +
                                                              '${(orderController.orderType == 'take_away' || (orderController.deliverySelectIndex == 0 ? restController.restaurant.freeDelivery : true)) ? 'free'.tr : _charge != -1 ? PriceConverter.convertPrice(orderController.deliverySelectIndex == 0 ? _charge : _deliveryCharge) : 'calculating'.tr}'
                                                          //  'Category ID: ${restController.restaurant.id}\n'+
                                                          //  'includeDelivery: ${restController.restaurant.categoryInfo[0].includeDelivery}\n'
                                                          )
                                                      : Text(
                                                          'delivery_charge'.tr +
                                                              ': ' +
                                                            '${(restController.restaurant.categoryInfo[0].includeDelivery == 1) ? 'include_delivery'.tr : (orderController.orderType == 'take_away' ? 'free'.tr : (orderController.deliverySelectIndex == 0 ? restController.restaurant.freeDelivery : _charge != -1 ? PriceConverter.convertPrice(_deliveryCharge) : 'calculating'.tr))}'
                                ...
                                children: [
                                  Container(
                                   ...
                                   width:MediaQuery.of(context).size.width*.45,
                                    ...
                                    padding: EdgeInsets.all(
                                        Dimensions.PADDING_SIZE_SMALL),
                                        // child: !orderController.isSaved
                                        child: !orderController.isLoading
                                        ? CustomButton(
                                            buttonText: 'save_order'.tr,
                                            onPressed: () {
                                              //  is_saved = true;
                                              is_saved = 1;
                                              _isPlaceorderButton = false;
                                              ...
                                              if (_orderAmount <
                                                ...
                                                showCustomSnackBar(
                                                   ...
                                                   {
                                                showCustomSnackBar(
                                                    'digital_payment_payment_method_is_not_for_saving_order'
                                                        .tr);
                                              }
  ...
  if (isSuccess) {
      ...
      if (Get.find<OrderController>().paymentMethodIndex == 0 ||
        ...
        Get.offNamed(
           RouteHelper.getOrderSuccessRoute(orderID, 'success', amount, is_saved));
      } else if(Get.find<OrderController>().paymentMethodIndex == 1) {
        if (GetPlatform.isWeb) {
          Get.back();
          String hostname = html.window.location.hostname;
          String protocol = html.window.location.protocol;
          String selectedUrl =
          '${AppConstants.BASE_URL}/payment-mobile?order_id=$orderID&customer_id=${Get.find<UserController>().userInfoModel.id}&&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&amount=$amount&status=&is_saved=$is_saved';
